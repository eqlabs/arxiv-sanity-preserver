...
class Config(object):
    ...
    thumbs_dir = os.path.join('static', 'thumb')
    # intermediate pickles
    tfidf_path = 'data/tfidf.p'
    meta_path = 'data/tfidf_meta.p'
    sim_path = 'data/sim_dict.p'
    user_sim_path = 'data/user_sim.p'
    # sql database file
    db_serve_path = 'data/db2.p' # an enriched db.p with various preprocessing info
    database_path = 'data/as.db'
    serve_cache_path = 'data/serve_cache.p'
    
    beg_for_hosting_money = 1 # do we beg the active users randomly for money? 0 = no.
    banned_path = 'data/banned.txt' # for twitter users who are banned
    tmp_dir = 'tmp'
